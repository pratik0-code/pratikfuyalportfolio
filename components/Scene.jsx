...
/* ==========================================================================
   ...
   and signal pulses) wrapped in a wireframe cage, orbit rings and satellites,
   plus a particle field. Reacts to the mouse, scroll and the active theme.
   Three.js is imported lazily on the client so it never runs during SSR.
   ========================================================================== */
...
export default function Scene() {
  ...
  useEffect(() => {
    ...
    import('three')
      .then((THREE) => {
        if (cancelled) return;
        dispose = createScene(THREE, canvas);
      })
      .catch((error) => {
...
function createScene(THREE, canvas) {
  ...
  const rig = new THREE.Group();   // positioned by layout + scroll
  const spin = new THREE.Group();  // rotated by time + mouse
  rig.add(spin);
  scene.add(rig);
  ...
  const particleMat = new THREE.PointsMaterial({ size: 0.035, transparent: true, opacity: 0.6, sizeAttenuation: true });
  ...
  scene.add(new THREE.AmbientLight(0xffffff, 0.7), key, rim);

  /* Theme colours -------------------------------------------------------- */
  const applyTheme = () => {
    const accent = cssColor(THREE, '--accent');
    const ink = cssColor(THREE, '--ink');
    const muted = cssColor(THREE, '--ink-muted');
  ...
  const onScroll = () => { scrollProgress = window.scrollY / window.innerHeight; };
